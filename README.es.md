...
| **`Bash` en una lente "READ-ONLY" es un vector de escritura** — `design-lens-taste`/`design-lens-motion` nunca necesitaron `Bash` (se quitó); `design-lens-impeccable`/`design-lens-a11y` sí necesitan una llamada de shell puntual y documentada cada una (`context.mjs`, cachear guidelines) sin otra herramienta para hacerlo | Se mantuvo `Bash` solo donde hay una llamada real documentada. **Limitación conocida:** `disallowedTools` en un subagente solo bloquea herramientas enteras, no patrones de comando (`sed -i`, redirects) — así que en estas dos, el read-only durante el diagnóstico está **forzado por prompt, no por herramienta**. Para una garantía dura, añade un `permissions.deny` a nivel de proyecto (p. ej. `Bash(sed -i:*)`, `Bash(rm:*)`) en `settings.json`. |
