...
platform, public-vs-private (private → `seo` add-on off), live browser. Resolve owner-only decisions
(scope, light-fix vs full redesign, brand constraints) in one `AskUserQuestion` batch. State what you
skip and why. **Do not ask anything the code can answer.**

---

## Step 1 — `audit-first` **[GATE · redesigns only]**

**Agent:** `design-audit-first`. **Trigger:** the target already exists.
...
flatness hypotheses). **PASS = the artifact exists**, or "skipped — greenfield" stated explicitly.
...
5. Write `.design-review/references.md` (sources, patterns, combined direction, dials, and a one-line
   "bar" for what alive-vs-flat means *for this target*).

**PASS = `references.md` exists with ≥3 tagged patterns + dials + the bar. Without it, STOP** — there is
nothing for the verdict to judge "alive" against.
...
**Flag correct-but-generic structure** as a vitality finding (feeds 3b). Fix routing:
`polish`/`layout`/`colorize`/`animate`/`typeset`/`harden`/`bolder`/`quieter`/`distill`/`delight`/
`clarify`/`adapt`/`optimize` — each has its own command.

### 3b — `design-lens-taste` (routes `design-taste-frontend` §11 + §14) — **anti-templated GATE**
Routes to **§11 REDESIGN PROTOCOL** (§11.A detect mode, §11.B audit-before-touching) and closes with
**§14 FINAL PRE-FLIGHT CHECK**. The **3 dials** are **pre-set from `references.md`** — do NOT ask the
user. The gate is constructed from the skill's §11/§14 output: **FAILS if the output could be any SaaS
template**. Exit criterion: "this could only be THIS product." Anti-templated items are **P1 vitality
findings**, pre-selected. Landing-specific rules (hero/eyebrow/marquee/images) are ADVISORY on
dashboards/product-UI; cross-cutting rules (em-dash, fake numbers, consistency, anti-templated) always
apply. Fix routing: §11.D modernisation levers + §14 pre-flight.
...
signature motion moment in `references.md` (`<ref>`); return concrete Before/After"*) — bypasses the
...
at least one memorable motion moment tied to a step-2 reference. **Hover-only = FAIL.** Mapping: missing
signature → P1; broken hygiene → P1/P2; timing refinement → P3. `review-animations` runs only if
installed (graceful).

### 3d — `design-lens-a11y` (routes `web-design-guidelines` with guidelines pre-fetched)
**First: WebFetch** the guidelines and cache to `.design-review/web-guidelines.md` (avoids the skill's
"which files?" prompt). Pass `target` + cached guidelines + prior-lens changes in the same invocation.
AA contrast (≥4.5:1, ≥3:1 large/UI), visible focus, full keyboard reach, correct roles/labels, logical
headings, meaningful `alt`, reduced-motion honored by the new signature motion. **Every WCAG A/AA
...
> lens — force review mode, grant `WebSearch`), `review-animations` (only if installed), `seo` (public
> targets only), a mobile-design skill (RN/Expo). They sharpen the result but are not gates.

**PASS = all 4 core lenses returned findings, merged into one `[skill]`-tagged list.**
...
polish. Each item: one-line description, skill tag(s), `file:line`, **owning fix command** (suite→phase
mapping), recommended fix. Present a multi-select checklist (`AskUserQuestion`, `multiSelect: true`) with
**P1 + anti-templated + signature-motion pre-marked** — those are why the target was flat. Apply only
what's chosen.
...
Route each chosen item to its **owning fix command**:
...
contrast → 3d + verdict). Surface genuinely new findings in a short follow-up batch; don't re-litigate
settled items.
...
**Agent:** `design-vitality-verdict`. The only place alive-vs-flat is actually settled.

1. Render the real target live (light/dark/mobile) via `agent-browser`. Screenshots are ground truth.
2. **Diff against `.design-review/references.md`:** for each pattern that was meant to land, did it land
   or regress to the mean during build?
3. Check the bar: **house layer** present? **density/bento** (no scroll for the key content)?
   **signature motion moment fires** (verify live, not from code)? **typography with a point of view**
   (the plan's 2+ roles present, not the framework default stack)?
4. Core Web Vitals (LCP ≤ 2.5s / CLS ≤ 0.1 / INP ≤ 200ms / TTFB) — perf is a UX gate.
5. Emit **`alive` / `templated` / `flat`** → `.design-review/verdict.json` (the hook reads it).
   **`alive` requires ALL bar checks true AND no reinforcement gate failed.** No live browser → the
   verdict is provisional and conservatively `templated` (you can't claim "alive" for a design no one
   looked at).
...
`alive` or explain why the bar can't be met (then the owner decides).
...
Every finding flows forward; nothing is discarded between steps. When two skills flag the same issue,
merge into one item and list both (`[impeccable, web-design-guidelines]`) — higher confidence,
pre-selected. Keep the most specific description (actual ratio, path, component).
